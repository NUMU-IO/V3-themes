/**
 * Genova translation lookup.
 *
 * WHY THIS EXISTS — the host does not ship theme translations. `ByotThemeBoundary`
 * builds the mount context with `themeSettings`, `storeData`, `page` and
 * `locale`, but never `translations`. The SDK's `useTranslation()` reads exactly
 * that field, so in practice `t(key, fallback)` always returns the fallback and
 * every theme's `locales/*.json` is inert at runtime. (The plugin does emit them
 * into `dist/manifest.json`, which is why they look wired.)
 *
 * Measured: rendering the home template under `locale: "ar"` produced identical
 * English output to `locale: "en"`, down to the character count.
 *
 ...
 *   2. this theme's locale file for the active locale (with English filling any
 *      gap — `pickTranslations` does that),
 ...
 * key — `footer.trust_shipping_title` shipped to a shopper. Ours behaves the
 * same way by design, to stay a drop-in replacement.
 */
