...
/**
 * emp-product-detail — the PDP body, ported from V2 EmpProductDetailPage.
 *
 * Layout: a split grid — a vertical stack of every product photo on one
 * side (each framed with the empire 2px border + offset shadow, decorative
 ...
 */
export default function EmpProductDetail({
  instance,
  sectionId,
}: SectionRenderProps) {
  ...
  return (
    <section
      className="bg-[var(--emp-cream)] pb-20 md:pb-0"
      data-emp-section={sectionId}
    >
      <div className="grid md:grid-cols-2">
        {/* Image column — vertical stack */}
        <div className="relative bg-[var(--emp-cream)] overflow-hidden">
          <div aria-hidden="true" className="hidden md:block absolute top-20 -end-12 w-40 h-40 bg-[var(--emp-amber)]/40 emp-blob pointer-events-none" />
          <div aria-hidden="true" className="hidden md:block absolute bottom-32 -start-16 w-52 h-52 bg-[var(--emp-navy)]/10 emp-blob emp-blob-delay-2 pointer-events-none" />
          <div aria-hidden="true" className="hidden md:block absolute top-6 start-6 w-32 h-32 opacity-40 pointer-events-none emp-dot-grid" />

          <Link
            to="/products"
            aria-label="Back to shop"
            className="absolute top-4 start-4 z-20 w-10 h-10 rounded-full bg-[var(--emp-dark)] text-[var(--emp-amber)] flex items-center justify-center hover:opacity-80 transition-opacity shadow-[3px_3px_0_var(--emp-amber)]"
          >
          ...
          {hasImages ? (
            <div className="relative z-10 flex flex-col gap-4 md:gap-5 p-4 md:p-6 lg:p-8">
              {images.map((img, i) => (
                <div
                  key={`${img.url}-${i}`}
                  className="relative bg-white rounded-2xl border-2 border-[var(--emp-dark)] overflow-hidden shadow-[6px_6px_0_var(--emp-dark)]"
                >
                  <img
                    ref={(el) => {
                    ...
                    className="w-full h-auto object-cover emp-img-zoom"
                  />
                </div>
              ))}
            </div>
            ...
            <div className="relative z-10 flex flex-col items-center justify-center p-8 min-h-[40vh]">
              <div className="w-48 h-48 sm:w-64 sm:h-64 rounded-2xl bg-white border-2 border-[var(--emp-dark)] shadow-[6px_6px_0_var(--emp-dark)] flex items-center justify-center">
                <ShoppingBag size={48} className="text-[var(--emp-dark)]/30" aria-hidden="true" />
              </div>
            </div>
          )}
        </div>
        ...
        <div className="emp-wavy-bg p-6 sm:p-8 md:p-12 lg:p-16">
          <div className="max-w-md mx-auto w-full md:sticky md:top-6">
            {/* Breadcrumb */}
            <nav
              aria-label="Breadcrumb"
              className="emp-label text-[11px] text-[var(--emp-dark)]/60 mb-4 flex items-center flex-wrap gap-1"
            >
              <Link to="/" className="hover:text-[var(--emp-dark)] transition-colors">
                {homeLabel}
              </Link>
              <ChevronLeft size={12} aria-hidden="true" className="rotate-180 rtl:rotate-0" />
              <Link to="/products" className="hover:text-[var(--emp-dark)] transition-colors">
                {shopLabel}
              </Link>
              <ChevronLeft size={12} aria-hidden="true" className="rotate-180 rtl:rotate-0" />
              <span className="text-[var(--emp-dark)]/80 truncate max-w-[160px]">
                {product.name}
              </span>
            </nav>

            <h1 className="emp-heading text-2xl sm:text-3xl md:text-4xl lg:text-5xl text-[var(--emp-dark)]">
              {product.name}
            </h1>

            {/* Price */}
            <div className="flex items-baseline gap-3 mt-3 md:mt-4 flex-wrap">
              <span className="emp-heading text-xl sm:text-2xl md:text-3xl text-[var(--emp-dark)]">
                <Money amount={activePrice} currency={product.currency} />
              </span>
              {hasDiscount && (
                <>
                  <span className="text-base sm:text-lg text-[var(--emp-dark)]/40 line-through">
            ...
            {!inStock ? (
              <div className="mt-4">
                <span className="emp-label text-red-600 text-sm">
                  {outOfStockLabel}
                </span>
              </div>
            ) : (
              <div className="mt-4 inline-flex items-center gap-2">
                ...
                <span className="emp-label text-green-800 text-xs">{inStockLabel}</span>
              </div>
            )}

            {/* Description */}
            {product.description && (
              <p className="text-[var(--emp-dark)]/70 text-sm mt-6 leading-relaxed">
                {product.description}
              </p>
            )}
            ...
            {product.options && product.options.length > 0 && (
              <div className="mt-8 space-y-6">
                {product.options.map((opt) => {
                  ...
                  return (
                    <div key={opt.name}>
                      <span className="emp-label text-[var(--emp-dark)]/60 mb-3 block uppercase">
                        {opt.name}
                        {chosen ? ` — ${chosen}` : ""}
                      </span>
                      <div
                        ...
                        {opt.values.map((v) => {
                          ...
                          return (
                            <button
                              ...
                              className={`emp-size-btn ${isSelected ? "active" : ""} ${
            ...
            {/* Quantity */}
            <div className="mt-6">
              <span className="emp-label text-[var(--emp-dark)]/60 mb-3 block">
                {quantityLabel}
              </span>
              <div className="inline-flex items-stretch rounded-full border-2 border-[var(--emp-dark)] overflow-hidden bg-[var(--emp-cream)]">
                <button
                  type="button"
                  aria-label="Decrease quantity"
                  onClick={() => setQuantity((q) => Math.max(q - 1, 1))}
                  disabled={quantity <= 1}
                  className="w-11 h-11 flex items-center justify-center text-[var(--emp-dark)] hover:bg-[var(--emp-dark)] hover:text-[var(--emp-amber)] transition-colors disabled:opacity-30 disabled:hover:bg-transparent disabled:hover:text-[var(--emp-dark)]"
                >
                  <Minus size={16} aria-hidden="true" />
                </button>
                <span
                  className="emp-heading text-base text-[var(--emp-dark)] min-w-[3rem] flex items-center justify-center border-x-2 border-[var(--emp-dark)] select-none"
                  aria-live="polite"
                >
                  {quantity}
                </span>
                <button
                  ...
                  className="w-11 h-11 flex items-center justify-center text-[var(--emp-dark)] hover:bg-[var(--emp-dark)] hover:text-[var(--emp-amber)] transition-colors"
                >
            ...
            <button
              ...
              className={`emp-btn w-full mt-8 rounded-full py-4 text-[13px] ${
                added
                  ? "bg-green-600 text-white border-green-600"
                  : !inStock
                    ? "opacity-50 cursor-not-allowed emp-btn-filled"
                    : "emp-btn-filled"
              }`}
            >
              {added ? (
            ...
            {/* Trust badges */}
            <div className="mt-8 pt-6 border-t border-[var(--emp-dark)]/15 grid grid-cols-3 gap-3">
              {badges.map((b, i) => {
                ...
                return (
                  ...
                  >
                    <Icon size={18} className="text-[var(--emp-dark)]" aria-hidden="true" />
                    <span className="emp-label text-[10px] text-[var(--emp-dark)]">
                      {b.label}
                    </span>
                    <span className="text-[10px] text-[var(--emp-dark)]/60 leading-tight">
                      {b.desc}
                    </span>
                  </div>
                );
      ...
      <div className="md:hidden fixed bottom-0 inset-x-0 z-40 bg-[var(--emp-cream)] border-t-2 border-[var(--emp-dark)] px-4 py-3 flex items-center gap-3 shadow-[0_-2px_8px_rgba(0,0,0,0.08)]">
        <div className="flex flex-col leading-tight shrink-0">
          ...
          {hasDiscount && (
            <span className="text-[10px] text-[var(--emp-dark)]/40 line-through">
        ...
        <button
          ...
          onClick={handleAdd}
          disabled={addDisabled || loading}
          className={`emp-btn flex-1 rounded-full py-3 text-[12px] ${
            added
              ? "bg-green-600 text-white border-green-600"
              : !inStock
                ? "bg-[var(--emp-dark)]/20 text-[var(--emp-dark)]/60 border-[var(--emp-dark)]/20 cursor-not-allowed"
                : "emp-btn-filled"
          }`}
        >
          {added ? (
