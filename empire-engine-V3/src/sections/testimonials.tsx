      ...
      <div className="empire-container">
        ...
        <div className="empire-tgrid">
          {reviews.map(({ bid, block }) => {
            ...
            return (
              <div className="empire-tcard" key={bid}>
                <div className="empire-stars" aria-label={`${rating} من 5`}>
                ...
                <EditableText
                  as="p"
                  className="empire-tquote"
                  sectionId={id}
                  blockId={bid}
                  settingId="text"
                  value={`"${text}"`}
                />
                <div className="empire-tauthor">
                  <span className="empire-tavatar">{author[0]}</span>
                  <div>
                    <EditableText
                      as="p"
                      className="empire-tname"
                      sectionId={id}
                      blockId={bid}
                      settingId="author"
                      value={author}
...
const Star = ({ filled }: { filled: boolean }) => (
  <svg
    ...
    stroke="currentColor"
    strokeWidth="2"
    className={`empire-star${filled ? " is-filled" : ""}`}
    aria-hidden="true"
  >
