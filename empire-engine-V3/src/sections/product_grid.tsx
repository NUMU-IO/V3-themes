  ...
  return (
    <section className="empire-container" style={{ paddingBlock: "2.5rem" }}>
      ...
      <div className="empire-toolbar">
        <span className="empire-label">{visible.length} منتج</span>
        {s.show_sort !== false ? (
          <select
      ...
      {visible.length === 0 ? (
        <p className="empire-placeholder" style={{ paddingBlock: "3rem" }}>
          لا توجد منتجات مطابقة.
        </p>
      ) : (
        <div className="empire-grid" style={{ ["--cols" as string]: cols }}>
          {visible.map((p) => (
