 ...
 * picker, quantity stepper, trust guarantees, related rail), re-skinned to the
 * skeuomorphic look via the `vn-*` utility classes that theme.css re-maps to
 * the skeuomorphic palette (framed image, tactile buttons, warm surfaces).
 *
 * Data/actions are SDK-native: the host wraps the PDP in ProductProvider so the
 * current product comes from `useProductOptional()`; variant pick from
 * `useVariantSelection()`; add-to-cart drives `useCart().addItem` via the chosen
 * variant id; related rail from `useRelatedProducts()`.
 *
 * Null-safety: with no product in context (editor / not-found slug) it renders a
 * graceful placeholder with a "Browse products" link instead of crashing.
 */
export default function SkeuProductDetail({ instance }: SectionRenderProps) {
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12">
          ...
          <div>
            <motion.div
              ...
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              className="relative overflow-hidden skeu-img-frame rounded-xl aspect-[3/4] mb-3"
            ...
            >
              ...
              {hasDiscount && (
                <span className="absolute top-3 start-3 skeu-badge px-2.5 py-1 rounded-lg text-[10px] font-bold">
                  -{discountPercent}%
                </span>
              )}
            </motion.div>
                ...
                {images.map((img, i) => (
                  <button
                    ...
                    className={
                      ...
                      (i === activeImage ? "opacity-100" : "opacity-60 hover:opacity-90")
                    }
                        ...
                        <button
                          ...
                          className={
                            ...
                            (isColor
                              ...
                              : "min-w-[44px] h-10 px-3 text-sm font-bold ") +
                            (active ? "skeu-chip-active " : "text-foreground ") +
