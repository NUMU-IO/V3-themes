 ...
 * `minifyWhitespace: false` even when the option is set explicitly. Identifiers
 * get shortened and syntax compacted (`autoplay: !0`), but every block comment
 * and all 2-space indentation survives — which is why the whole V3 fleet shipped
 * unminified and Lighthouse charged vionne 37 KiB of `unminified-javascript`
 * before this pass was written.
 *
 * Vite's reasoning for the carve-out is that a published ES library is normally
 ...
 *
 * Why not `@import "./fonts.css"` and let Vite handle it: in LIBRARY mode Vite
 * inlines every asset referenced from CSS as a base64 data URI and ignores
 * `build.assetsInlineLimit` while doing it. Measured on Genova: theme.css went
 * 51 KB → 368 KB, it became render-blocking at that size, and — worst of all —
 * inlining destroys `unicode-range`, so every shopper downloaded the six Arabic
 * faces even on a page with no Arabic character on it.
 *
 * Writing the CSS text ourselves at generateBundle time keeps the url() opaque
 * to Vite. `_deploy-bundle-r2.py` walks dist/ recursively and already maps
 * `.woff2`, so `dist/fonts/*` reaches the CDN beside theme.css and the relative
 * URL resolves.
 */
function emitFonts(): PluginOption {
  return {
    ...
    generateBundle(_options, bundle) {
      ...
      // exist" — the SSR pass still emits a stray `style-*.css`.
      ...
      // Fail the build rather than no-op. Without `enforce: "post"` above,
      // theme.css is not in the bundle yet and this loop silently matches
      // nothing — producing a theme that renders in a system fallback on every
      // device. Visually plausible, completely wrong, invisible in CI.
      if (!patched) {
 ...
 * and the host's `error_template_url` resolves to a 404.
 *
 * Verified fleet-wide during the Genova build: vionne, bazar and luxury-minimal
 * all declare the same path and none of them ship the file, so every V3 theme
...
/**
 * Teen V3 build config.
 *
 * Tailwind-in-bundle: src/main.tsx imports src/theme.css, whose `@tailwind`
 * directives + the `tn-*` component classes are compiled by Vite's PostCSS
...
export default defineConfig({
  ...
  build: {
    ...
    cssCodeSplit: false,
    /**
     * Emit assets as FILES, never base64. See emitFonts() above for the
     * measured cost of getting this wrong.
     */
    assetsInlineLimit: 0,
  },
  // 5174, not 5173: Genova already owns 5173 and both dev servers get run
  // side by side when comparing the two themes.
  server: { port: 5174 },
});
