...
/**
 * Gilded product-detail section.
 *
 * Faithful port of the proven Vionne V3 PDP (gallery-left / info-right, the
 * `vn-*` utility classes) re-skinned to the Gilded palette via the re-mapped
 * `--vn-*` tokens in theme.css. Data/actions are SDK-native: the host wraps
 * the PDP in ProductProvider so the current product comes from
 * `useProductOptional()`; variant pick + selected variant from
 * `useVariantSelection()`; add-to-cart drives `useCart().addItem`; related
 * rail from `useRelatedProducts()`.
 *
 ...
 */
export default function GildedProductDetail({ instance }: SectionRenderProps) {
      ...
      <div className="bg-background min-h-[50vh] flex items-center justify-center px-4 py-20 text-center">
        <div className="max-w-sm">
          ...
          <p className="text-sm text-[var(--vn-muted)] mb-7 leading-relaxed">
            {localized(locale, "Open a product from the shop to see its details here.", "افتح منتج من المتجر علشان تشوف تفاصيله هنا.")}
          </p>
          ...
          <div>
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              {/* The SDK's <AddToCartButton> owns the loading/disabled/error
                  ...
                  without re-implementing it. We only supply Gilded styling. */}
              <AddToCartButton
                product={product}
