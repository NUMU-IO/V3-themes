    ...
    return (
      <div className="mt-4 flex items-center justify-between gap-3 text-sm">
        ...
        <button
          ...
          disabled={busy}
          onClick={() => void remove()}
          className="opacity-70 underline disabled:opacity-50"
        >
    ...
    <form className="mt-4" onSubmit={submit}>
      <div className="flex gap-2">
        <input
          ...
          className="min-w-0 flex-1 border border-black/15 bg-transparent px-3 py-2 font-mono text-xs"
        />
        <button type="submit" disabled={busy || !code.trim()} className="rounded-full bg-[var(--by-espresso,#3a2418)] text-[var(--by-cream,#fdf8ee)] font-semibold uppercase tracking-wide px-4 py-2 text-xs disabled:opacity-50">
