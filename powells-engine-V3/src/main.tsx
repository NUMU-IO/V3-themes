...
import { RequestFormDialog, RequestSideTab } from "./lib/request-form";
...
function ThemeApp({ currentTemplate }: { currentTemplate: string }) {
  ...
  return (
    ...
    >
      ...
      <WishlistDrawer />
      <RequestFormDialog />
      {globals.show_request_tab !== false && <RequestSideTab />}
    </div>
  );
}
...
if (import.meta.env.DEV && typeof window !== "undefined" && typeof document !== "undefined") {
  ...
  if (rootEl && !rootEl.dataset.numuMounted) {
    ...
    void import("./lib/dev-fixtures").then(({ DEV_CART, devDetail, devPage, devSearch }) => {
      ...
      const hostFetch = browser ? browser.fetch.bind(browser) : fetch;
