 ...
 * `useTranslation()` reads exactly that field, so `t(key, fallback)` always
 * returns the fallback and every theme's `locales/*.json` is inert at runtime.
 * The plugin DOES emit them into `dist/manifest.json`, which is exactly why
 * they look wired when they are not.
 *
 ...
 * key — `cart.summary_title` shipped to a shopper.
 */
...
export function useT(): TFunction {
  ...
  return useCallback<TFunction>(
    (key, fallback) => {
      ...
      return typeof local === "string" && local ? local : fallback;
    },
    [hostT, messages],
  );
}
