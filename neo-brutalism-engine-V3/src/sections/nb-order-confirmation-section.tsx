...
export default function NBOrderConfirmationSection({ instance }: SectionRenderProps) {
  ...
  const subtitle =
    asString(s.subtitle) ||
    localized(locale, "Thank you for your order. We'll send you the order details via WhatsApp.", "شكراً لطلبك. سنرسل لك تفاصيل الطلب عبر واتساب.");
        ...
        >
          ...
          <div className="nb-card rounded-lg p-4 sm:p-5 md:p-6 mb-6 sm:mb-7 text-start">
            <div className="space-y-4 text-sm">
              ...
              <div className="h-px bg-[var(--vn-border)]" />
              <div className="flex justify-between">
                <span className="text-xs sm:text-sm text-[var(--vn-muted)]">{localized(locale, "Estimated Delivery", "موعد التسليم المتوقع")}</span>
                <span className="text-xs sm:text-sm font-black text-[var(--vn-ink)]">{localized(locale, "3-5 business days", "٣-٥ أيام عمل")}</span>
              </div>
          ...
          {showWhatsApp && (
            <div className="nb-card rounded-lg p-4 sm:p-5 mb-6 sm:mb-7 text-start">
              <div className="flex items-center gap-2 mb-2.5 sm:mb-3 vn-label text-[10px] sm:text-[11px]">
                ...
                <span className="text-sm font-black text-whatsapp">{localized(locale, "WhatsApp Message", "رسالة واتساب")}</span>
              </div>
              <div className="bg-[var(--vn-band)] rounded-md p-3 sm:p-4 text-xs sm:text-sm text-[var(--vn-ink)] leading-relaxed border-[2px] border-foreground">
                <p>{localized(locale, "Hello! 👋", "أهلاً بيك! 👋")}</p>
                <p className="mt-1">
                  {localized(locale, "Your order ", "طلبك رقم ")}
                  <strong className="font-black text-[var(--vn-ink)]">{orderNumber}</strong>
                  {localized(locale, " has been received successfully.", " اتسجّل بنجاح.")}
                </p>
                {typeof total === "number" && total > 0 && (
                  <p className="mt-1">
                ...
                <p className="mt-1">{localized(locale, "We'll deliver it within 3-5 business days. If you have any questions, contact us here.", "هنوصّلهولك خلال ٣-٥ أيام عمل. لو عندك أي استفسار، كلّمنا من هنا.")}</p>
