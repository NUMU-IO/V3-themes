...
/**
 * rs-header — faithful V3 port of V2 MashkalStoreHeader
 * (numu-egyptian-bazaar/src/components/store/manshet/MashkalStoreHeader.tsx),
 * re-plumbed onto the V3 SDK seam.
 *
 * The Mashkal masthead chrome: a PAPER (`vn-header` = --rs-page-bg) sticky bar
 ...
 * Shop / About / Contact).
 */
export default function RsHeader({ instance, sectionId }: SectionRenderProps) {
  ...
  // Default OFF — V2 Mashkal ships no header announcement strip (it was
  ...
  // (3) else the V2 Mashkal default set (Shop / About / Contact).
        ...
        <div className="container mx-auto px-4 h-14 md:h-16 grid grid-cols-[1fr_auto_1fr] items-center gap-3 relative">
            ...
            <nav className="hidden md:flex items-center gap-5">
              {nav.map((link, i) => (
                <Link
                  ...
                  to={link.to}
                  className="vn-label rs-ink-link"
                >
                  {link.label}
                </Link>
              ))}
          ...
          >
            {logoUrl ? (
              ...
              <span className="rs-masthead text-xl md:text-2xl truncate block">
              ...
              enableMiniCart ? (
                ...
                >
                  ...
                  {cartCount > 0 && (
                    ...
                    <span
                      key={cartCount}
                      className="rs-badge-stamp absolute -top-1 -end-2 min-w-[16px] h-[16px] px-1 rounded-full bg-[var(--rs-primary)] text-white text-[9px] font-bold flex items-center justify-center"
                    >
                      {cartCount}
                    </span>
                  )}
                ...
                >
                  <ShoppingBag size={18} />
                  {cartCount > 0 && (
                    <span className="absolute -top-1 -end-2 min-w-[16px] h-[16px] px-1 rounded-full bg-[var(--rs-primary)] text-white text-[9px] font-bold flex items-center justify-center">
                      {cartCount}
                    </span>
                  )}
                </Link>
      ...
      >
        ...
        <aside className="vn-drawer-panel">
          <div className="flex items-center justify-between px-5 h-14 border-b border-[var(--vn-border)]">
            <span className="rs-masthead text-lg">{brandName}</span>
            <button
              type="button"
