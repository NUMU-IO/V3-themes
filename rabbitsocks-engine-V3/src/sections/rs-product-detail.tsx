...
/**
 * Rabbitsocks product-detail section.
 *
 * Faithful port of the proven Vionne V3 PDP (gallery-left / info-right, the
 * `vn-*` utility classes re-mapped onto Rabbitsocks's quiet-luxury palette via
 * theme.css), re-plumbed on the V3 SDK. The host wraps the PDP in
 * ProductProvider so the current product comes from `useProductOptional()`;
 * variant pick + selected variant from `useVariantSelection()`; add-to-cart
 * drives `useCart().addItem` via the chosen variant id; related rail from
 * `useRelatedProducts()`.
 *
 ...
 */
export default function RsProductDetail({ instance }: SectionRenderProps) {
          ...
          <div>
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              <AddToCartButton
                ...
                soldOutLabel={
                  <>
                    <ShoppingBag size={16} /> {localized(locale, "Sold out", "خلص من المخزن")}
                  </>
                }
