    ...
    return (
      <div className="mt-4 flex items-center justify-between gap-3 text-sm">
        ...
        <button
          ...
          disabled={busy}
          onClick={() => void remove()}
          className="text-[var(--vn-muted)] underline disabled:opacity-50"
        >
    ...
    <form className="mt-4" onSubmit={submit}>
      <div className="flex gap-2">
        <input
          ...
          className="min-w-0 flex-1 border border-[var(--vn-border)] bg-transparent px-3 py-2 font-mono text-xs"
        />
        <button type="submit" disabled={busy || !code.trim()} className="vn-btn vn-btn-filled px-4 py-2 text-xs disabled:opacity-50">
