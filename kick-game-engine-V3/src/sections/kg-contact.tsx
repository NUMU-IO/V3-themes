 ...
 * (breadcrumb, message form, real merchant contact channels from
 * `useShop().social_links`, business hours, optional map) re-palette'd to Kick
 * Game via theme.css. Never invents placeholder channels — hides any channel
 * the merchant didn't configure.
 */
  ...
  return (
    <div className="bg-background min-h-[60vh]">
      <div className="container mx-auto px-4 py-10 md:py-14 max-w-5xl">
        ...
        <h1 className="kg-heading text-3xl md:text-4xl mb-3">{title}</h1>
        ...
        <div className="grid md:grid-cols-2 gap-8 md:gap-12 mt-8 md:mt-12">
          ...
          <div>
            {submitted ? (
              <div className="bg-[var(--vn-band)] rounded p-8 text-center">
                <h3 className="kg-heading text-lg mb-1.5">{successTitle}</h3>
            ...
            {showMap && mapEmbedUrl && (
              <div className="pt-6">
                <iframe
                  ...
                  className="w-full h-64 border border-[var(--vn-border)] rounded"
                  loading="lazy"
