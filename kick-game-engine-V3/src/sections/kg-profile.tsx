 ...
 *
 * Ported from the proven vionne V3 profile (sidebar [avatar, stats, nav,
 * logout] + Orders / Addresses / Settings tabs with full CRUD) re-plumbed on
 * the V3 SDK and re-palette'd to Kick Game via the `vn-*` tokens in theme.css.
 * Never blank / never crashes: logged-out renders the auth guard; logged-in
 * but empty renders empty states; loading shows spinners.
 */
...
export default function KGProfile({ instance }: SectionRenderProps) {
  ...
  const inputClass =
    "w-full h-10 px-3 border border-[var(--vn-border)] bg-transparent text-sm rounded focus:border-[var(--vn-ink)] outline-none transition-colors";
      ...
      <div className="container mx-auto px-4 py-8 md:py-12">
        ...
        <div className="flex flex-col lg:flex-row gap-8 lg:gap-12">
          ...
          <div className="lg:w-64 shrink-0">
            <div className="mb-6">
              <div className="w-12 h-12 rounded-full bg-[var(--vn-band)] flex items-center justify-center text-[var(--vn-ink)] text-lg font-bold mb-3">
                {initial}
              </div>
              <h1 className="kg-heading text-base text-[var(--vn-ink)]">{fullName}</h1>
                ...
                return (
                  <button
                    ...
                    className={
                      ...
                      (active
                        ? "text-[var(--vn-ink)] font-bold bg-[var(--vn-band)]"
            ...
            {activeTab === "orders" && (
              <div>
                ...
                {loadingOrders ? (
                  ...
                  <div className="border border-[var(--vn-border)] rounded overflow-hidden">
                    {orders.map((order, i) => (
                      <div
                        key={order.id}
                        className={
